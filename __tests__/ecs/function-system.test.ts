...
test('into_system_type_id_consistency', () => {
    function testfn<In extends SystemInput, Out, Marker, T extends IntoSystemTrait<In, Out, Marker>>(fn: T) {
        const reference_system = define_system({ system: function reference_system() { }, params: [] })
    ...
    const function_system = define_system({ system: function function_system() { }, params: [] })

    testfn(function_system as any);
})