...
export const on_timer = (duration_ms: number) => {
    const timer = new Timer(duration_ms, TimerMode.Repeating);
    return defineCondition(b => b.res(Time), function on_timer(time) {
        timer.tick(time.v.delta);
...
export const once_after_delay = (duration: number) => {
    const timer = new Timer(duration, TimerMode.Once);
    return defineCondition(b => b.res(Time), function once_after_delay(time) {
        timer.tick(time.v.delta);
        return timer.just_finished();
    })
}
...
export const repeating_after_delay = (duration: number) => {
    const timer = new Timer(duration, TimerMode.Repeating);
    return defineCondition(b => b.res(Time), function repeating_after_delay(time) {
        timer.tick(time.v.delta);
...
export const paused = defineCondition(b => b.res(Time), function paused(time) {
