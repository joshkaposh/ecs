...
export const event_update_system = define_system((b) => b.world().last_change_tick(), (world, last_change_tick) => {
    if (world.contains_resource(EventRegistry)) {
        world.resource_scope(EventRegistry, (world, registry) => {
            registry.value.run_updates(world, last_change_tick.value);

            const registry_should_update = registry.value.should_update;

            registry.value.should_update = registry_should_update === ShouldUpdateEvents.Always ?
...
export const event_update_condition = define_condition((b) => b.res_opt(EventRegistry), (maybe_signal) => {
    if (maybe_signal) {
        return ShouldUpdateEvents.Waiting !== maybe_signal.should_update;
    } else {
        return true;
    }
})