...
import { type Component, type ComponentId, type ComponentInfo, Components, type Resource, type ResourceId, ComponentTicks, Tick, relative_to } from "../component";
import { Storages } from "../storage";
import { type Entity, Entities, EntityDoesNotExistDetails, EntityLocation, index } from "../entity";
import { type Bundle, BundleInserter, Bundles, BundleSpawner, InsertMode } from "../bundle";
import { QueryState, QueryTupleToQueryData, QueryTupleToQueryFilter, Access, FilteredAccess } from "../query";
...
import { type System, type SystemMeta, IntoSystem, RunSystemError, defineParam, Commands } from "../system";
...
import { type TypeId, type Instance, unit, debug_assert, all_tuples } from "../util";
...
//     componentId(component: Component): Option<ComponentId> {
...
//     component_info(component: Component): Option<ThinComponentInfo> {
//         const id = this.componentId(component);
//         return id != null ? this.#components.getInfo(id) : null;
//     }
...
//         return this.#storages.resources.get(component_id)?.delete()?.[0] as InstanceType<R>;
//     }
...
function scheduleRun<T extends Schedule>(world: World, schedule: T) {
    schedule.run(world);
    return schedule;
}

type SpawnResult<T, C extends boolean> = C extends false ? Option<void> : T;
...
class World {
    ...
    registerSystem<I extends any, O>(_system: IntoSystem<I, O>) {

    }
    registerRequiredComponents<T extends Component, R extends Component<new () => any>>(_component: T, _required: R) {
        // this.try_register_required_components(component, required);
    }
    registerRequiredComponentsWith<T extends Component>(_component: T, _constructor: new () => any) {
        // this.try_registry_required_components_with(component, constructor);
    }

    tryRegisterRequiredComponents(_component: Component, _required: Component<new () => any>) {
        // this.try_register_required_components_with(component, required.constructor);
    }
    tryRegisterRequiredComponentsWith<T extends Component, R extends Component>(_component: T, _required: R, _constructor: () => InstanceType<R>) {
 ...
 */
     ...
     * @returns a [`EntityWorldMut`] reference.
     */
    spawn(...bundles: BundleInput) {
        this.flush();
        const change_tick = this.#change_tick;
        const entity = this.#entities.alloc();
        const bundle_spawner = BundleSpawner.new(bundles, this, change_tick);
        let entity_location = bundle_spawner.spawnNonExistent(entity, bundles);
        if (!this.#command_queue.is_empty()) {
            this.flush();
    ...
    // spawnOld(...bundles: BundleInput): EntityWorldMut {
    //     this.flush();
    //     const change_tick = this.#change_tick;
    //     const entity = this.#entities.alloc();
    //     const bundle = Bundles.dynamicBundle(bundles);
    //     const bundle_spawner = BundleSpawner.new(bundle, this, change_tick)
    //     let entity_location = bundle_spawner.spawnNonExistent(entity, bundle);
    //     if (!this.#command_queue.is_empty()) {
    //         this.flush();
    //         entity_location = this.#entities.get(entity) ?? EntityLocation.INVALID;
    //     }
    ...
    spawnBatch<C extends boolean>(batch: BundleInput[], collect = false as C): SpawnResult<Entity[], C> {
        // this.flush();
        return TODO('World.spawnBatch', batch, collect);
        // const change_tick = this.#change_tick;

        // const length = batch.length;
        // this.#entities.reserve(length);

        // const spawner = BundleSpawner.new(batch[0], this, change_tick);
        // spawner.reserveStorage(length);

        // const spawn_fn = (element: any) => spawner.spawn(element);
        // if (collect) {
        //     return toflatTuples(batch, spawn_fn) as any;
        // } else {
        //     return flatTuples(batch, spawn_fn) as any;
        // }
    }

    // spawnBatchFast<C extends boolean>(batch: BundleInput[], collect = false as C) {
    //     this.flush();
    //     const change_tick = this.#change_tick;
    ...
    //     const spawner = BundleSpawner.newFast(batch[0], this, change_tick);
    //     spawner.reserveStorage(length);

    //     const spawn_fn = (element: any) => spawner.spawn(element);
    //     if (collect) {
    //         return all_tuples_into_flattened(batch, spawn_fn) as any;
    //     } else {
    //         return all_tuples(batch, spawn_fn) as any;
    //     }
    // }

    // spawnBatchIter<C extends boolean>(batch: ArrayLike<BundleInput>, collect: C = false as C): SpawnResult<Entity[], C> {
    //     this.flush();
    //     const change_tick = this.#change_tick;
    //     const length = batch.length;
    //     this.entities.reserve(length);
    ...
    //     const spawner = BundleSpawner.newOld(bundle, this, change_tick);
    //     spawner.reserveStorage(length);

    //     const spawn_fn = (element: any) => spawner.spawn(element);
    //     const iterator = iter(batch);

    //     if (collect) {
    //         return all_tuples_iter_into_flattened(iterator, spawn_fn) as any;
    //     } else {
    //         return all_tuples_iter(iterator, spawn_fn) as any;
    //     }
    // }

    tryInsertBatchIfNew<const B extends any[] | Bundle>(batch: [Entity, B][]) {
     ...
     */
    tryInsertBatch<const B extends any[] | Bundle>(batch: [Entity, B][], insert_mode: InsertMode = InsertMode.Replace): Result<undefined, TryInsertBatchError> {
        ...
        if (invalid_entities.length === 0) {
            ...
            return new TryInsertBatchError(bundle_type.type_id, invalid_entities);
        }
    }

    insertBatch<const B extends [Entity, any[] | Bundle][]>(batch: B, insert_mode: InsertMode = InsertMode.Replace) {
        this.flush();
            ...
            if (first_location) {
                ...
                for (const [entity, bundle] of batch_iter) {
                    ...
                    if (location) {
                        ...
                        throw new Error(`Could not insert a bundle (of type ${bundle_type.type_id}) for entity ${entity}, which ${EntityDoesNotExistDetails}`)
                    }
                }

            } else {
                throw new Error(`Could not insert a bundle (of type ${bundle_type.type_id}) for entity ${first_entity}, which ${EntityDoesNotExistDetails}`)
            }
        }
    }

    insertBatchIfNew<const B extends [Entity, any[] | Bundle][]>(batch: B) {
    ...
    */
     ...
     */
    query<const D extends any[]>(data: D) {
 ...
 */
    ...
    queryFiltered<const D extends any[], const F extends any[]>(data: D, filter: F): QueryState<QueryTupleToQueryData<D>, QueryTupleToQueryFilter<F>> {
    ...
    removeResource<R extends Resource>(resource: R): Option<InstanceType<R>> {
        const component_id = this.#components.resourceId(resource);
        return component_id == null ? null : this.#storages.resources.get(component_id)?.delete()?.[0] as InstanceType<R>;
    }

    hasResource(resource: Resource): boolean {
        const resource_id = this.#components.resourceId(resource);
        return resource_id == null ? false :
            this.#storages.resources.get(resource_id)?.isPresent ?? false;
    }
    ...
    getResource<R extends Resource>(resource: R): Option<InstanceType<R>> {
        const id = this.#components.resourceId(resource);
    ...
    getResourceById<R extends Resource>(resource_id: ResourceId): Option<InstanceType<R>> {
        return this.#storages
            .resources
            .get(resource_id)
    ...
    getResourceMutById<R extends Resource>(resource_id: ResourceId): Option<Mut<R>> {
        const tuple = this.#storages.resources.get(resource_id)?.getWithTicks();
        return tuple ?
    ...
    /**
     * Inserts a new resource by its id.
     */
    insertResourceById<R extends Resource>(resource_id: ResourceId, value: InstanceType<R>) {
        this.__initializeResourceInternal(resource_id).set(value, this.#change_tick);
    }
     ...
     */
    getResourceOrInit<R extends Resource>(resource: R): Mut<R> {
        const change_tick = this.#change_tick;
        const last_change_tick = this.#last_change_tick;
    ...
    iterResources() {
        return this.#storages.resources.iter().filter_map(([id, data]) => {
            ...
            return ptr ? [this.#components.getInfo(id)!, ptr] as [ComponentInfo, {}] : undefined;
        })
    }
    ...
    removeResourceById(component_id: ComponentId): Option<unit> {
        return this.#storages.resources.getMut(component_id)?.deleteAndDrop() as unit;
    }
    ...
    tryRunSchedule(label: ScheduleLabel) {
        return this.tryScheduleScope(label, scheduleRun);
    }

    runSchedule(label: ScheduleLabel) {
        this.scheduleScope(label, scheduleRun);
    }
